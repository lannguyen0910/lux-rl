...
def make_city_actions(game_state: Game, missions: Missions, DEBUG=False) -> List[str]:
    ...
    for city_tile in city_tiles:
        ...
        cluster_leader = game_state.xy_to_resource_group_id.find(
            tuple(city_tile.pos))
        cluster_unit_limit_exceeded = \
            game_state.xy_to_resource_group_id.get_point(tuple(city_tile.pos)) <= len(
                game_state.resource_leader_to_locating_units[cluster_leader])
        ...
        nearest_resource_distance = game_state.distance_from_collectable_resource[
